...
- **P5 已完成**：schema 驗證、版本化遷移、隔離區、匯入驗證。
- **P4b 已完成**：渲染改為畫面註冊表 ＋ store 訂閱。
  目前共 221 條測試。
...
### 3. 單向資料流
`事件 → domain 計算 → 改 store → 訂閱者自動重繪`。
UI 模組之間**不得互相呼叫對方的 render**。

**新增畫面**：在自己的檔案裡呼叫 `registerView({ id, deps, render })`，
宣告依賴哪些 store 狀態即可。不要去改 `renderAll()`，也不要改分頁切換的判斷。

**改資料**：用重新賦值（`records = [...records, rec]`），
全域存取器會自動通知訂閱者、畫面自動更新，不必呼叫 `renderAll()`。

**就地修改是例外**：`records.push(x)`、`settings.osm = true` 這類寫法
存取器攔不到，畫面不會更新。只在批次迴圈中使用（逐筆重新賦值是 O(n²)），
並在迴圈後呼叫 `store.touchMany([...])`。
...
- **一律走 `src/core/migrations.js`**，在陣列末尾加一筆，並補測試涵蓋
  「需要轉換」與「已經轉換過」兩種情況。每條遷移必須**冪等**。
- **遷移先於驗證**。舊格式資料在遷移前本來就不符合現行形狀，
  先驗證會把遷移所需的線索抹掉（開發時實際踩過：驗證把 `split: undefined`
  補成 `null`，遷移就認不出舊資料了）。
- **驗證寧可寬鬆也不要嚴格**。這些是使用者累積多年的帳本，
  過度嚴格造成的資料損失比放行一筆奇怪資料更嚴重。
  只檢查「少了就會讓下游壞掉」的欄位。
- **無法使用的資料送進隔離區，不要刪除**（`storage.js` 的 `quarantine()`）。
- **任何資料問題都要讓使用者看得到**（設定頁的 `renderDataHealth()`）。
- **所有外部進來的資料都要過驗證器**：匯入的 JSON／CSV、雲端同步下載的內容。

## 已知缺陷

- **Drive 同步衝突**：目前接近後寫覆蓋，多裝置同時編輯可能覆蓋彼此的變更。
  待 P5b 改為依 `updatedAt` 逐筆合併。
- **沒有版本回復**：隔離區能保住壞掉的資料，但還沒有「回到昨天的版本」的能力。
  待 P5b。
