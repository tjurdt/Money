...
import { calculateDiscountPlan, normalizeDiscountRate } from '../../src/domain/discount.js';
...
describe('normalizeDiscountRate — 折扣率正規化', () => {
  it('接受小數、一位數與百分數三種寫法', () => {
    expect(normalizeDiscountRate(0.8)).toBe(0.8);
    expect(normalizeDiscountRate(8)).toBe(0.8);
    expect(normalizeDiscountRate(80)).toBe(0.8);
    expect(normalizeDiscountRate(85)).toBe(0.85);
  });
  it('1 與 10 都代表不打折', () => {
    expect(normalizeDiscountRate(1)).toBe(1);
    expect(normalizeDiscountRate(10)).toBe(1);
  });
  it('無效輸入回傳 null，0 回傳 0', () => {
    expect(normalizeDiscountRate(0)).toBe(0);
    expect(normalizeDiscountRate(-1)).toBeNull();
    expect(normalizeDiscountRate('x')).toBeNull();
    expect(normalizeDiscountRate(101)).toBeNull();
  });
});

describe('calculateDiscountPlan — 基本情況', () => {
  it('無規則時原價即實付，逐項淨額不變', () => {
    const p = calculateDiscountPlan(ITEMS, 0, []);
  ...
  it('整單八折 → 折 80，且逐項按比例攤提', () => {
    const p = calculateDiscountPlan(ITEMS, 0, [
      { id: 'd1', type: 'order_percent', mode: 'stack', rate: 8 },
    ]);
  ...
  it('整單折抵定額 50 → 依淨額比例分攤到各品項', () => {
    const p = calculateDiscountPlan(ITEMS, 0, [
      { id: 'd2', type: 'fixed', mode: 'stack', amount: 50 },
    ]);
...
describe('calculateDiscountPlan — 門檻與上限', () => {
  ...
  it('未知的規則型別會被忽略', () => {
    const p = calculateDiscountPlan(ITEMS, 0, [{ id: 'x', type: 'bogus', mode: 'stack', rate: 1 }]);
    expect(p.finalTotal).toBe(400);
  });
});
...
describe('calculateDiscountPlan — 手動覆寫實付金額', () => {
  ...
  it('覆寫值高於原價時不套用（防呆）', () => {
    const p = calculateDiscountPlan(ITEMS, 0, [], 9999);
...
describe('calculateDiscountPlan — 無品項（整筆記帳）', () => {
  it('沒有品項時以 baseAmount 當成單一「整單」列', () => {
    const p = calculateDiscountPlan([], 500, [
      { id: 'd1', type: 'order_percent', mode: 'stack', rate: 9 },
    ]);
  ...
  it('金額為 0 時不會產生 NaN', () => {
    const p = calculateDiscountPlan([], 0, [
      { id: 'd2', type: 'fixed', mode: 'stack', amount: 50 },
    ]);
